package com.gkm.techconn.datos

import com.gkm.techconn.modelo.productosDatos

class productosInformacion {

    companion object {
        val productoList = listOf<productosDatos>(
            productosDatos(
                "BALANZA 20KG C/PLATO REDONDO ACERO SP20 VERDE",
                "4-32-15",
                "PLATINUM",
                "UND",
                "613",
                "I",
                "11.90",
                "https://static.wixstatic.com/media/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg/v1/fill/w_281,h_300,al_c,q_80,enc_auto/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg"
            ),
            productosDatos(
                "CINTA MASKING 1\" X 37M MULTIUSO DON PEPE",
                "1-1-53",
                "ABRO",
                "UND",
                "210",
                "L",
                "0.87",
                "https://static.wixstatic.com/media/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg/v1/fill/w_281,h_300,al_c,q_80,enc_auto/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg"
            ),
            productosDatos(
                "TIRADERA # P22217-128 SS TUBO VACIA (10 MM) (53.1001)",
                "5-1-660",
                "PLATINUM",
                "UND",
                "2400",
                "I",
                "0.40",
                "https://static.wixstatic.com/media/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg/v1/fill/w_281,h_300,al_c,q_80,enc_auto/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg",
            ),
            productosDatos(
                "AMARRA NYLON 12X4.8MM NEGRO",
                "1-2-27",
                "BEST VALUE",
                "CIENTO",
                "20",
                "L",
                "4.95",
                "https://static.wixstatic.com/media/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg/v1/fill/w_281,h_300,al_c,q_80,enc_auto/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg"
            ),
            productosDatos(
                "NIVELADOR DE MUEBLE 10 CM (ZOCALO) (AJ097)",
                "1-12-54",
                "PLATINUM",
                "UND",
                "450",
                "I",
                "1.68",
                "https://static.wixstatic.com/media/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg/v1/fill/w_281,h_300,al_c,q_80,enc_auto/1ae92d_f59c95b7991849c89e5839e5447e9ae8~mv2.jpg"
            ),
            productosDatos(
                "TIRADERA # P21203-128 BRN PUNTOS (51.0991B)",
                "5-1-509",
                "PLATINUM",
                "UND",
                "580",
                "I",
                "0.92",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "ESCUADRA REFUERZO INDUMA 38X38X38 MM ZINC (FUNDA 50 UNDS)",
                "7-10-17",
                "INDUMA",
                "SET",
                "20",
                "L",
                "16.00",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "LIJA EN DISCO #100",
                "4-24-22",
                "NORTON",
                "UND",
                "110",
                "L",
                "0.76",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "CINTA MASKING 1\" X 60 YDS P/ACABADO #2090 AZUL",
                "1-1-61",
                "INDUMA",
                "UND",
                "119",
                "L",
                "3.15",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "LLAVE P/FREG. C/GANZO P/PARED \"SINCLEC\" (145B1) PALETA",
                "6-1-113",
                "SINCLEC",
                "UND",
                "64",
                "L",
                "4.15",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "Q. QUEMADOR IND.B-05HNC MINI RED.REF. S/LLAVE V.CLARO",
                "6-9-28",
                "PLATINUM",
                "UND",
                "0",
                "I",
                "5.50",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "GARRUCHA NYLON PLATAFORMA 1 1/2 GRANEL (CA9632)",
                "1-19-33",
                "PLATINUM",
                "JGO",
                "298",
                "I",
                "1.10",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "BISAGRA PRESION NEGRA TWN (SHC798)",
                "7-6-67",
                "PLATINUM",
                "PAR",
                "9433",
                "I",
                "0.60",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "LIJA EN ROLLO P/METAL #60  R/TELA X-86",
                "4-24-37",
                "FANDELLI",
                "ROLLO",
                "0",
                "L",
                "152.50",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "TIRADERA P/VIDRIO # PH015-1200 RECTANGULAR NIQ",
                "5-1-96",
                "PLATINUM",
                "UND",
                "103",
                "I",
                "19.50",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "PICAPORTE DE SIMBRA 3 AC DE LUJO (T47804)",
                "7-7-17",
                "PLATINUM",
                "UND",
                "413",
                "I",
                "1.29",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "TIRADERA # H15888-96 NEGRA",
                "5-1-20",
                "PLATINUM",
                "UND",
                "11",
                "I",
                "0.99",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "BOTON NEGRO REDONDO #51.7071 -25 MM",
                "5-2-90",
                "PLATINUM",
                "UND",
                "1648",
                "I",
                "0.69",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "RODAMIENTO P/RIEL CORREDIZA P/CLOSET (YM001)",
                "1-12-13",
                "PLATINUM",
                "SET",
                "486",
                "I",
                "3.75",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "NIVELADOR NIQ. 10CM (SL134)",
                "1-12-55",
                "PLATINUM",
                "UND",
                "2159",
                "I",
                "0.95",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            ),
            productosDatos(
                "CERRAD. P/PUERTA VIDRIO (CP) (408)",
                "7-4-110",
                "PLATINUM",
                "UND",
                "604",
                "I",
                "1.10",
                "https://www.adslzone.net/app/uploads-adslzone.net/2019/04/borrar-fondo-imagen.jpg"
            )

        )
    }

}